...
app.get('/api/quiz-history', (req, res) => {
  ...
  const historyArr = Object.values(quizHistory)
  ...
  if (fs.existsSync(questionsDir)) {
    ...
    files.forEach(file => {
      ...
      if (quizData.setName && quizData.questions) {
        ...
        console.log(`Loaded ${quizData.questions.length} questions from ${file} (${quizData.setName})`);
      } else {
        ...
        console.log(`Loaded ${questions.length} questions from ${file} (legacy format)`);
      }
    });
...
io.on('connection', (socket) => {
  ...
  socket.on('join_room', (data) => {
    ...
    console.log(`Player ${playerName} (Student ID: ${studentId}) joined room ${roomId}`);
  });
  ...
  socket.on('start_quiz', (roomId) => {
    ...
    const currentQuestionObj = rooms[roomId].questions[rooms[roomId].currentQuestionIndex];
  ...
  socket.on('submit_answer', (data) => {
    ...
    if (!rooms[roomId] || !rooms[roomId].isActive || !rooms[roomId].players[socket.id]) {
    ...
    if (isCorrect) {
      ...
      const pointsEarned = Math.floor(currentQuestionObj.points * timeBonus * streakMultiplier);
    ...
    console.log(`Player ${player.name} submitted answer ${answerId} for question in room ${roomId}`);
    
    // Check if all players have answered
    const allPlayersAnswered = Object.values(rooms[roomId].players).every(p => {
  ...
  socket.on('leave_room', (roomId, deleteRoom = false) => {
    ...
    if (rooms[roomId].hostId === socket.id) {
      ...
      if (deleteRoom) {
        ...
        // Delete the room
        delete rooms[roomId];
        console.log(`Room ${roomId} was deleted because the teacher requested it`);
      } else {
        console.log(`Teacher temporarily left room ${roomId} but kept it active`);
      }
    }
    ...
    for (const roomId in rooms) {
      if (rooms[roomId].players[socket.id]) {
        ...
        if (rooms[roomId].hostId === socket.id) {
          ...
          console.log(`All players left room ${roomId}, but teacher is still connected`);
        }
      }
    }
  });
...
function endQuestion(roomId) {
  ...
  Object.values(rooms[roomId].players).forEach(player => {
    const hasAnswered = player.answers.some(a => a.questionId === currentQuestionObj.id);
    if (!hasAnswered) {
...
function moveToNextQuestion(roomId) {
  ...
  const nextQuestionObj = rooms[roomId].questions[rooms[roomId].currentQuestionIndex];
  ...
  console.log(`Moving to question ${rooms[roomId].currentQuestionIndex + 1} in room ${roomId}`);
...
app.get('/api/quizzes', (req, res) => {
  const availableQuizzes = Object.keys(questionSets).map(quizId => {
    ...
    return {
      ...
      firstQuestion: quizSet.questions.length > 0 ? quizSet.questions[0].question : null
    };
  });
