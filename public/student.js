...
joinForm.addEventListener('submit', (e) => {
  ...
  // Handle different room ID formats - might be quiz ID or full room ID
...
socket.on('player_joined', data => {
  ...
  if (players.length > 0) {
    ...
    players.forEach(player => {
      ...
      playerItem.style.cssText = 'font-weight: 500; white-space: nowrap; max-width: 100%; overflow: hidden; text-overflow: ellipsis; word-break: break-word;';
...
socket.on('answer_result', data => {
  ...
  // Show feedback
  feedbackText.textContent = isCorrect ? `Correct! +${pointsEarned}` : 'Incorrect!';
...
socket.on('question_ended', data => {
  ...
  const resultStreakContainer = document.getElementById('resultStreakContainer');
  ...
  if (hasAnswered) {
    ...
    if (lastAnswer && lastAnswer.isCorrect) {
      ...
      answerResultMsg.innerHTML = '<i class="bi bi-check-circle"></i> Your answer was correct!';
    } else {
      answerResultMsg.className = 'alert alert-danger';
      answerResultMsg.innerHTML = '<i class="bi bi-x-circle"></i> Your answer was incorrect!';
    }
  } else {
    answerResultMsg.className = 'alert alert-warning';
    answerResultMsg.innerHTML = '<i class="bi bi-exclamation-triangle"></i> You did not answer in time!';
  }
});
